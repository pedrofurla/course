...
-- A functor which in NOT applicative
-- data Const k a = Const k

-- instance Applicative (Const x) where
--   pure :: a -> Const x a
--   pure x = _todo1
...
-- g :: ExactlyOne (Int -> Int)
-- g = ExactlyOne (+ 2)

-- x :: ExactlyOne Int
-- x = ExactlyOne 3

-- proveComposition :: Bool
...
instance Applicative List where
  ...
  -- (<*>) fs as = foldRight (\f bs -> map f as ++ bs ) Nil fs
...
-- Similiar to fmap but with an additional argument
-- (<$>) :: (a -> b) -> k a -> k b
...
-- Hmm sequence is basically more generic seqOptional
