...
sequence (a :. t) = --(\t' -> ) <$> (sequence t)
  let
    kappend :: Applicative k1 => k1 (b -> List b -> List b)
    kappend = pure (:.)
  in
    kappend <*> a <*> sequence t
...
filtering f (a :. as) =
  pure (\b as' -> if b then a :. as' else as') <*> f a <*> filtering f as
--  let
--    fa :: k Bool
--    fa  = f a
--    kappend :: k (Bool -> List a -> List a)
--    kappend = pure (\b as' -> if b then a :. as' else as')
--  in
--    kappend <*> fa <*> filtering f t
