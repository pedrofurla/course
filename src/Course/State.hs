...
findM p =
  foldRight
    (\a foa -> p a >>= (\b -> if b then pure (Full a) else foa))
    (pure Empty)
...
-- In contrast, a sad number (not a happy number) is where the sum of the square of
-- its digits never reaches 1 because it results in a recurring sequence.
--
...
-- >>> isHappy 7
-- True
-- 49 -> 16 + 81 = 97 -> 81 + 49 = 130 -> 1 + 9 + 0 = 10 -> 1
--
-- >>> isHappy 42
-- False
--
...
isHappy n =
  let xs = produce f (fromInteger n :: Int)
      f n' = sum $ map (square . digitToInt) (show' n')
   in contains 1 $ firstRepeat xs
